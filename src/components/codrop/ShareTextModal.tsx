...
export function ShareTextModal({
  ...
  async function share() {
    setBusy(true);
    try {
      const drop = await createTextDrop(text);
      setText("");
      onCreated(drop);
    } catch (error) {
      ...
      <DialogContent className="max-w-3xl">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            className="h-[45vh] min-h-64 w-full resize-y rounded-xl border border-border bg-secondary/30 p-4 font-mono text-sm leading-relaxed text-foreground shadow-inner placeholder:text-muted-foreground/70 focus:outline-none focus:ring-2 focus:ring-ring"
          />
